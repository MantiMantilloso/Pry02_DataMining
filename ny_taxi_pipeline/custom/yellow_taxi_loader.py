...
def _upsert_partition(engine, df: pd.DataFrame, schema: str, table: str):
    """
    Idempotent export via COPY:
      1. Creates schema + table if they don't exist
      2. Deletes the (source_month, service_type) partition
      3. Bulk-inserts via COPY in chunks
    ...
    try:
        with raw_conn.cursor() as cursor:
            ...
            # 2. Idempotency — delete existing partition before reloading
            cursor.execute(f"""
            ...
            # 3. COPY in chunks — na_rep='' maps NaN/NA → NULL in Postgres
        ...
        raw_conn.commit()  # single commit — DELETE + COPY are atomic
...
def run(*args, **kwargs):
    ...
    def to_table_schema(df: pd.DataFrame, year: int, month: int) -> pd.DataFrame:
        # 1. Lowercase source columns
        df.columns = [c.lower() for c in df.columns]

        # 2. Fill any source column missing in this parquet
        ...
        # 4. Fill green-only columns with NA (not in yellow source)
        ...
        # 7. Return canonical column order
